...
<div class="d-flex w-100">
    ...
    <div class="content p-4 w-100 pb-5" style="background-color: #f8fafc; min-height: 100vh;">
        ...
        <div class="mb-4">
            <h4 class="fw-bold" style="color: #0f172a;">Anggota</h4>
            <p class="text-muted">Kelola akun siswa yang terdaftar di perpustakaan</p>
        </div>
        ...
        <div class="table-wrapper">
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <div class="input-group" style="max-width: 500px;">
                    ...
                    <input type="text" id="searchAnggota" class="form-control border-start-0 ps-0 shadow-none" placeholder="Cari anggota (nama, email, NISN)...">
                </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover mt-3 align-middle text-nowrap">
                    <thead style="color: #475569; font-size: 14px;">
                        <tr>
                            <th class="fw-medium pb-2 border-bottom-0">Nama Lengkap</th>
                            <th class="fw-medium pb-2 border-bottom-0">NISN</th>
                            <th class="fw-medium pb-2 border-bottom-0">Email</th>
                            <th class="fw-medium pb-2 border-bottom-0">Peran</th>
                    ...
                    <tbody style="border-top: 1px solid #e2e8f0;">
                        ...
                        <tr class="anggota-row">
                            <td class="py-3 searchable" style="color: #334155;">{{ $user->name }}</td>
                            <td class="py-3 searchable" style="color: #475569;">{{ $user->nisn ?? '-' }}</td>
                            <td class="py-3 searchable" style="color: #475569;">{{ $user->email }}</td>
                            <td class="py-3" style="color: #475569;"><span class="badge bg-secondary">Siswa</span></td>
                            <td class="py-3 text-end">
                                <button class="btn btn-sm text-secondary p-1" data-bs-toggle="modal" data-bs-target="#editModal{{ $user->id }}" title="Edit">
                                ...
                                <form action="{{ route('admin.anggota.destroy', $user->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus akun anggota ini?');">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <div class="modal-content rounded-4 border-0 shadow">
                                    <form action="{{ route('admin.anggota.update', $user->id) }}" method="POST">
                                        ...
                                        <div class="modal-header border-bottom-0 pb-0 mt-2 px-4">
                                            <h5 class="modal-title fw-bold fs-5" style="color: #0f172a;">Edit Anggota</h5>
                                            <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <div class="modal-body px-4 pt-1">
                                            <p class="text-muted mb-4" style="font-size: 14px;">Perbarui data anggota perpustakaan</p>

                                            <div class="mb-3">
                                                <label class="form-label" style="font-size: 14px; color: #334155;">Nama Lengkap <span class="text-danger">*</span></label>
                                                <input type="text" name="name" class="form-control" value="{{ $user->name }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label" style="font-size: 14px; color: #334155;">NISN (Opsional)</label>
                                                <input type="text" name="nisn" class="form-control" value="{{ $user->nisn }}">
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label" style="font-size: 14px; color: #334155;">Email <span class="text-danger">*</span></label>
                                                <input type="email" name="email" class="form-control" value="{{ $user->email }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label" style="font-size: 14px; color: #334155;">Password Baru (Opsional)</label>
                                                <input type="password" name="password" class="form-control" placeholder="Isi untuk mengganti password">
                                                <small class="text-muted" style="font-size: 12px;">Kosongkan jika tidak ingin mengubah password.</small>
                                            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content rounded-4 border-0 shadow">
            <form action="{{ route('admin.anggota.store') }}" method="POST">
                @csrf
                <div class="modal-header border-bottom-0 pb-0 mt-2 px-4">
                    <h5 class="modal-title fw-bold fs-5" style="color: #0f172a;">Tambah Anggota Baru</h5>
                    <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body px-4 pt-1">
                    <p class="text-muted mb-4" style="font-size: 14px;">Masukkan rincian data siswa/anggota baru</p>
                    ...
                    <div class="mb-3">
                        <label class="form-label" style="font-size: 14px; color: #334155;">Password Sementara <span class="text-danger">*</span></label>
...
<script>
    // Search filter
    document.getElementById('searchAnggota').addEventListener('keyup', function(e) {
        let value = e.target.value.toLowerCase();
        let rows = document.querySelectorAll('.anggota-row');
        rows.forEach(row => {
            let matches = false;
